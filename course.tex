...
\begin{itemize}
\item $\textcircled{\small{1}}$和$\textcircled{\small{2}}$为什么成立？
\item 求$V^{\pi}(\vec S)$对$\pi$矩阵的梯度
\begin{align*}
...
\begin{frame}\frametitle{蒙特卡洛方法}
\resizebox{.8\linewidth}{!}{
    \begin{minipage}{\linewidth}
蒙特卡洛智能体与环境交互产生若干条状态，动作与收益序列$\boldsymbol{h}=[s_0,a_0,r_1,s_1,a_1,r_2,\dots,s_t,a_t,r_{t+1},\dots]$。在计算值函数时，蒙特卡洛方法利用经验池中$\boldsymbol{h}$带衰减累计收益的均值估计$V^{\pi}(s)=\mathbb{E}[G_t|s_t=s]$期望值，而能否准确估计值函数，则依赖算法高效地探索和利用经验。$N$表示从$s$出发的轨迹条数，$G_j(s)$表示第$j$条轨迹的带衰减累计收益值，可采用递增技巧计算$N$条经验轨迹的均值。
\begin{columns}
\column{.5\textwidth}
        \begin{align*}
            V_{N}(s) &= \frac{1}{N} \sum_{j=1}^{N} G_{j}(s) \\
            &=\frac{1}{N}\left(G_{N}(s)+\sum_{j=1}^{N-1} G_{j}(s)\right) \\
            &=\frac{1}{N}\left(G_{N}(s)+(N-1) V_{N-1}(s)\right) \\
            &=V_{N-1}(s)+\frac{1}{N}\left(G_{N}(s)-V_{N-1}(s)\right)
        \end{align*}
\column{.5\textwidth}
        \par 更一般的蒙特卡洛更新形式如下，其中$\alpha$为学习率。
        \begin{align*}
            V\left(s\right) &\leftarrow V(s)+\alpha\left(G_{t}-V(s)\right)\\
            \text{也即}~~V\left(s\right) &\leftarrow \alpha G_t+(1-\alpha)V\left(s\right)
        \end{align*}
\end{columns}
        \end{minipage}
}
...
\begin{frame}\frametitle{时序差分方法}
\resizebox{.8\linewidth}{!}{
    \begin{minipage}{\linewidth}
采用bootstrap及在线学习思想，它估计下一状态-动作值函数，用$\alpha$控制值函数更新的学习率，更新公式如下。其中$r_{t+1}+\gamma Q\left(s_{t+1}, a_{t+1}\right)$被称为TD目标（TD target），它与$Q\left(s_{t}, a_{t}\right)$作差称作TD误差（TD error）。
        \begin{align*}
          Q\left(s_{t}, a_{t}\right) &\leftarrow Q\left(s_{t}, a_{t}\right)+\alpha\left[\overbrace{\underbrace{r_{t+1}+\gamma Q\left(s_{t+1}, a_{t+1}\right)}_{\text{TD target}}-Q\left(s_{t}, a_{t}\right)}^{\text{TD error}}\right]
        \end{align*}

        \par 状态$s_{t+1}$依据不同的动作采样与$Q$值计算方法确定$Q(s_{t+1},a_{t+1})$，可导出Q学习与SARSA等变种算法。
        \end{minipage}
}
\end{frame}
...
\item Q学习
        \begin{align*}
             Q\left(s_{t}, a_{t}\right) &\leftarrow Q\left(s_{t}, a_{t}\right)+\alpha\left[r_{t+1}+\gamma \max_{a_{t+1}\in A}Q\left(s_{t+1}, a_{t+1}\right)-Q\left(s_{t}, a_{t}\right)\right]
        \end{align*}

        \item SARSA
        \begin{align*}
           Q\left(s_{t}, a_{t}\right) &\leftarrow Q\left(s_{t}, a_{t}\right)+\alpha\left[r_{t+1}+\gamma Q\left(s_{t+1}, a_{t+1}|a_{t+1}\sim \epsilon-greedy\right)-Q\left(s_{t}, a_{t}\right)\right]
        \end{align*}

        \item Expected SARSA
        \begin{align*}
            Q\left(s_{t}, a_{t}\right) \leftarrow Q\left(s_{t}, a_{t}\right)+\alpha\left[r_{t+1}+\gamma \mathbb{E}_{\pi}\left[Q\left(s_{t+1}, a_{t+1}\right) \vert s_{t+1}\right]-Q\left(s_{t}, a_{t}\right)\right]
            %摆出原始定义，再做一些转化%
        \end{align*}
\end{itemize}
        \par 实际计算将依据$\epsilon$-greedy策略采样出$M$个动作$a_{t+1}$，计算$M$个Q值的均值$\bar{Q}$替代期望$\mathbb{E}$运算，故Expected SARSA亦可写成以下形式。
        \begin{align*}
            Q\left(s_{t}, a_{t}\right) \leftarrow Q\left(s_{t}, a_{t}\right)+\alpha\left[r_{t+1}+\gamma \bar{Q}_{a_{t+1}\sim \epsilon-greedy}\left(s_{t+1}, a_{t+1}\right)-Q\left(s_{t}, a_{t}\right)\right]
        \end{align*}
        \end{minipage}
}
\end{frame}
...
\begin{frame}\frametitle{Double DQN}
        \begin{align*}
          Y^{DQN}&\equiv r+\gamma \max _{a^{\prime}} Q\left(s^{\prime}, a^{\prime} ; \theta_{i}^{-}\right)
        \end{align*}
        \par Double DQN和DQN一样，也有两个Q网络。DQN的第二个Q网络起目标网络的作用，DDQN的第二个网络发挥 了将选动作与评估该状态-动作值函数解耦的作用。当选定动作$a$后，评估值函数仍采用$Q\left(s^{\prime}, a^{\prime} ; \theta_{i}^{-}\right)$，但选动作则依据$\arg\max_{a^{\prime}\in A}Q\left(s^{\prime},a^{\prime},\theta_{i}\right)$，DDQN的TD目标为$Y^{DoubleDQN}$。$\theta_{i}$与$\theta_{i}^{-}$表示两个$Q$网络的参数。
        \begin{align*}
          Y^{DoubleDQN} &\equiv r+\gamma Q\left(s^{\prime}, \arg\max_{a^{\prime}\in A}Q(s^{\prime},a^{\prime},\theta_i) ; \theta_{i}^{-}\right)
        \end{align*}
\end{frame}

\begin{frame}\frametitle{PER. DQN}
随机优先抽样方法介于纯贪心抽样和均匀随机抽样之间，设转移样本$i$处的TD偏差为$\delta_i$，设定样本$i$的采样概率$P(i)$为
        \begin{align*}
            P(i)&=\frac{p_{i}^{\alpha}}{\sum_{k} p_{k}^{\alpha}}
        \end{align*}
        \par $p_{i}$为转移样本$i$的优先级，转移样本形如$\langle s_i, a_i, r_{i+1}, s_{i+1}, a_{i+1}\rangle$。指数$\alpha$指示优先级对采样概率的重要度，当$\alpha=0$时退化成均匀随机采样。有两种方法定义$p_i$，一种为$p_{i}=\left|\delta_{i}\right|+\epsilon$，$\epsilon$为小的正数以确保TD偏差为0的样本也有一定概率被抽中；第二种为$p_{i}=\frac{1}{\operatorname{rank}(i)}$，其中$\operatorname{rank}(i)$表示将所有样本按$|\delta_i|$排序存储后样本$i$的序号。用$\delta_i$确定$P(i)$，因为TD误差越大，样本越能让智能体“惊奇”，该样本包含的“信息量”越大。
\end{frame}

\begin{frame}\frametitle{Dueling DQN}
\resizebox{.8\linewidth}{!}{
    \begin{minipage}{\linewidth}
将全连接层拆分为二：一支计算状态值函数$\hat{V}(s)$，另一支计算关于动作优势值$\hat{A}(s,.)$，最后再汇合为$\hat{Q}(s,.)$状态-动作值函数。
        ...
        \end{align*}
        \par 为导出上式中$Q^{\pi}(s,a)$、$V^{\pi}(s)$和$A^{\pi}(s,a)$之间的关系，推导过程如下。
        \begin{align*}
            V^{\pi}(s)&=\sum_{a} \pi(s, a ; \theta) Q^{\pi}(s, a)\\
            \sum_{a} \pi(s, a ; \theta) V^{\pi}(s)&=\sum_{a} \pi(s, a ; \theta) Q^{\pi}(s, a)\\
            0&=\sum_{a} \pi(s, a ; \theta) \left[Q^{\pi}(s, a)-V^{\pi}(s)\right]
        \end{align*}
        \end{minipage}
}
\end{frame}
...
\begin{frame}\frametitle{Dueling DQN}
        \par 将项$Q^{\pi}(s, a)-V^{\pi}(s)$定义为优势函数$A^{\pi}(s,a)$，它度量出相较于平均动作，某动作$a$具有的相对优势。
        \begin{figure}[htbp]
            \centering\includegraphics[scale=.5]{../duelingdqn.pdf}
	    \caption{Dueling DQN架构}
        \end{figure}
\end{frame}
...
\item 蒙特卡洛方法估计梯度
\begin{align*}
\nabla J(\theta)&=\sum_{s} p_1(s)V^{\pi}(s)\\
&=\sum_{s} p_1(s)\sum_{x}\sum_{k=0}(\gamma T^{\pi})^{(k)}_{s\rightarrow x}\mathbb{E}_{\pi}\left[Q^{\pi}(x,a)\nabla \log \pi(x,a;\theta)\right]\\
& \cdots \\
&\approx \frac{1}{m}\sum_{i=1}^{m}\sum_{k=0}^{H_{i}}\left[\gamma^k Q(x,a)\nabla \log \pi(x, a ; \theta)|x_0\stackrel{(k)}{\longrightarrow}x\right]
\end{align*}
\end{itemize}
\end{frame}
